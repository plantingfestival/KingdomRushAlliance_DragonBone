-- chunkname: @C:\\Users\\dev02\\Desktop\\Customized KR5\\Kingdom Rush Alliance\\kr5\\kr3_game_scripts.lua
...
function scripts.tower_silver.update(this, store)
	...
	local function y_do_shot(attack, enemy, level)
		...
		if V.dist(tpos(this).x, tpos(this).y, enemy.pos.x, enemy.pos.y) <= a.range then
			...
			if attack.use_obsidian_upgrade and enemy.health and enemy.health.armor == 0 then
				b.bullet.damage_min = b.bullet.damage_max
			end
		...
		elseif store.tick_ts - a.ts > a.cooldown then
			...
			if targets then
				for _, e in ipairs(targets) do
					...
					if crowds and #crowds >= a.min_targets and amount < #crowds then
						amount = #crowds
						crowdsTarget = crowds
					end
				end
			...
			if store.tick_ts - ra.ts < ra.cooldown then
				...
				if targets then
					for i = #targets, 1, -1 do
						...
						if not U.is_inside_ellipse(t_pos, tpos(this), a.range) or U.is_inside_ellipse(t_pos, tpos(this), ra.min_range) or not this.tower.flip_x and (radian > 0 or radian < -math.pi / 2) or this.tower.flip_x and radian > -math.pi / 2 then
...
function scripts.hero_faustus.update(this, store)
	...
	while true do
		...
		if skill_ultimate.ts and store.tick_ts - skill_ultimate.ts >= ultimate_controller.cooldown then
			local target, targets, ultimatePos = U.find_enemy_with_search_type(store.entities, this.pos, 0, skill_ultimate.max_range, skill_ultimate.node_prediction, nil, nil, nil, nil, skill_ultimate.search_type, skill_ultimate.crowd_range, skill_ultimate.min_targets)
...
function scripts.hero_veznan.update(this, store)
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			if skill_ultimate.ts and store.tick_ts - skill_ultimate.ts >= ultimate_controller.cooldown then
				local target, targets, ultimatePos = U.find_enemy_with_search_type(store.entities, this.pos, 0, skill_ultimate.max_range, nil, nil, nil, nil, nil, skill_ultimate.search_type, skill_ultimate.crowd_range, skill_ultimate.min_targets)
...
function scripts.mod_chomp_bot_transformation.remove(this, store, script)
	...
	if target and source then
		local can_spawn = target.health.dead and target.death_transformation_entity_name and not source.health.dead and source.max_chomp_bots and not target._necromancer_entity_name and band(target.health.last_damage_types, bor(DAMAGE_EAT, DAMAGE_NO_SPAWNS)) == 0
...
function scripts.kr5_decal_black_baby_dragon.update(this, store)
	...
	local function find_enemy()
		local enemy = U.find_enemy_with_search_type(store.entities, this.pos, 0, attack.range, 0, attack.vis_flags, attack.vis_bans, nil, nil, U.search_type.custom, nil, nil, function(e1, e2)
			local p1 = e1.nav_path
			local p2 = e2.nav_path
			...
			if not expired then
				...
				while not this.motion.arrived do
					...
					elseif old_flip_sign then
						...
						flip_sign = enemy.pos.x < this.pos.x and -1 or 1
					end
				...
				delay_controller.entities = {
					controller1,
					controller2
				}
				delay_controller.delays = {
					fts(2),
					0.1
				}
