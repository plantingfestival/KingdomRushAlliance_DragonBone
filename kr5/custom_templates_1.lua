-- chunkname: @C:\\Users\\dev02\\Desktop\\Customized KR5\\Kingdom Rush Alliance\\kr5\\custom_templates_1.lua
...
tt.radian = -0.75 * math.pi
tt.radian_var = 0.1 * math.pi
...
tt.dodge.counter_attack.damage_max_config = {
	15,
	30
}
tt.dodge.counter_attack.damage_min_config = {
	10,
	20
}
...
tt.powers.spectral_communion.price = {
	175,
	175
}
...
tt.powers.promotion.promotion_index = {
	1
}
tt.powers.promotion.price = {
	150
}
...
tt.powers.single_extinction.price = {
	180,
	100,
	100
}
...
tt.modifier.vis_flags = F_MOD
tt.modifier.vis_bans = F_NIGHTMARE
...
function tt:filter_func(entity)
	return self.owner ~= entity and entity.tower.can_be_sold and table.contains(self.tower_kinds, entity.tower.kind) and not table.contains(self.excluded_templates, entity.template_name)
end
...
local scale = 1.6203703703703705
...
function tt.powers.chosen.on_power_upgrade(this, power_name, pow, store)
	this.render.sprites[4].hidden = nil
end

tt.powers.tentangles = E:clone_c("power")

function tt.powers.tentangles.on_power_upgrade(this, power_name, pow, store)
...
function tt.powers.goblin_launcher.on_power_upgrade(this, power_name, pow, store)
	if pow.level == 2 then
...
function tt.powers.better_crew.on_power_upgrade(this, power_name, pow, store)
...
tt.bullet.rotation_speed = 8.333333333333334 * FPS * math.pi / 30
tt.bullet.vis_bans = bor(F_FRIEND)
