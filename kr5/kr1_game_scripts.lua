-- chunkname: @C:\\Users\\dev02\\Desktop\\Customized KR5\\Kingdom Rush Alliance\\kr5\\kr1_game_scripts.lua
		...
		if this.unit.is_stunned then
			...
			if not a.disabled and store.tick_ts - a.ts >= a.cooldown then
				local towers = U.find_towers_in_range(store.entities, this.pos, a, function(t)
					return t.tower.can_be_mod and not U.has_modifiers(store, t, a.mod) and t.attacks and t.attacks.list[1] and t.attacks.list[1].cooldown and t.tower.kind ~= TOWER_KIND_BARRACK
				end)
			...
			if SU.check_unit_attack_available(store, this, a) then
				brk, sta = SU.entity_attacks(store, this, a)
...
function scripts.hero_magnus.update(this, store)
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			if not a.disabled and store.tick_ts - a.ts >= a.cooldown then
				local pos, crowd = U.find_enemy_crowd_position(store.entities, this.pos, a.min_range, a.max_range, a.vis_flags, a.vis_bans, nil, a.crowd_range, a.min_targets, true, U.position_type.node_floor_center, true)
...
function scripts.eb_sarelgaz.update(this, store, script)
	local ma = this.melee.attacks[1]
	local ta = this.timed_attacks.list[1]
...
function scripts.mod_denas_tower.insert(this, store)
	...
	if this.cooldown_factor and target.attacks and target.attacks.list[1].cooldown then
	...
	if target.shooters then
		for i, s in ipairs(target.shooters) do
			if s.attacks and s.attacks.list[1].cooldown then
