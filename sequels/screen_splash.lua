-- chunkname: @./sequels/screen_splash.lua
...
function screen:start_animation()
	...
	if features.show_simple_splash_logo then
	...
	splash_anim.pos = V.v(self.window.size.x / 2, self.window.size.y / 2)

	self.window:add_child(splash_anim, 1)
	splash_anim:start()

	if not features.show_simple_splash_logo then
		S:queue("GUISplash")
	end
end
